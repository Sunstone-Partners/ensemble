...
(just a count check) is the design goal, not enforcing continuation.
   ...
   -   traceability: subset of TASK_TRACEABILITY for this track's bead IDs only (keyed by bead id; empty object when TRD_MODE=false). A bead whose entry has is_test_task=true MUST be routed through the Reqnroll BDD path (resolve REQNROLL_CLI per tool-path-resolution skill; generate-bindings -> RED gate -> reqnroll-binding-specialist fills bodies -> RED proof) before its paired implementation bead can turn it green — same contract as implement-trd-task.yaml Steps 1a-1c. Do not implement -TEST beads with ad-hoc unit tests.
