...
description: Single-task primitive for implement-trd.yaml. Runs ONE task through implement -> review -> close (Reqnroll BDD path for TRD-NNN-TEST tasks) and emits a JSON summary. Non-recursive. (Codex skill for /ensemble:implement-trd-task)
user-invocable: true
