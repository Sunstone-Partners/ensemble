...
PASS (4.0+) or CONCERNS (3.0-3.9): save as normal.
FAIL (below 3.0): still save the PRD, with Status set to Draft -- unlike
create-prd, which refuses to save on FAIL. This command's entire purpose is
producing a starting draft for a human's later refine-prd pass; refusing to
save a low-scoring async draft would defeat that purpose. State the
FAIL/CONCERNS/PASS result plainly either way, and say clearly that a human
refine-prd pass is expected next given the score.
