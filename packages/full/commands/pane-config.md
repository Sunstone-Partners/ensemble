...
```bash
/ensemble:pane-config                        # Show current settings
/ensemble:pane-config multiplexer wezterm    # Set multiplexer
/ensemble:pane-config direction bottom       # Set split direction
/ensemble:pane-config percent 40             # Set pane size
/ensemble:pane-config floating true          # Enable Zellij floating panes
```
...
```
/ensemble:pane-config
```

**Use WezTerm with bottom split at 40%:**
```
/ensemble:pane-config multiplexer wezterm
/ensemble:pane-config direction bottom
/ensemble:pane-config percent 40
```

**Enable Zellij floating panes:**
```
/ensemble:pane-config multiplexer zellij
/ensemble:pane-config floating true
```
