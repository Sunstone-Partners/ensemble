...
---
<!-- Command: ensemble-create-trd-foreman | Version: 3.1.0 -->
...
3. Immediately after each `### N.M Sprint N` heading, write a **Shippable State:** line: one sentence describing the visible capability available once every task in that sprint is done (e.g., 'Users can log in with email/password; profile editing is not yet available'). Infrastructure-only statements ('scaffolding complete') are not acceptable.
4. For every story, emit a markdown table with required columns `ID | Task | Est. | Deps`
5. Set every Status cell to `[ ]` so Foreman creates dispatchable native tasks rather than completed tasks
6. Use dependencies that reference other task IDs exactly as strings in the Dependencies column
7. Ensure task rows are implementation-oriented and deterministic, not prose narratives
8. **CRITICAL**: First column header must be exactly `id` (lowercase) — not `ID`, `TRD ID`, or `Task ID`
9. **CRITICAL**: Second column must contain `task`, `description`, or `title`
