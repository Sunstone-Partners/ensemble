...
- **NFR-6 (Operability / observability):** The runtime must be observable from outside its own process. An operator must be able to determine, at any time and without reading source or attaching a debugger, (a) whether the extension loaded and which behaviors are active, (b) whether events are being received and dispatched, (c) what each invocation decided, and (d) whether the capabilities required for autonomous operation — a fix provider and an approval channel — are actually configured. Rationale: every failure mode in this design is fail-closed and silent, so "not loaded", "loaded but misconfigured", and "working correctly on a quiet day" are otherwise indistinguishable. A governance system that cannot be asked whether it is running cannot be operated, and will degrade to doing nothing without anyone noticing.
